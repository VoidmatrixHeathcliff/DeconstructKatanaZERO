--[=======================================================================[
    ...
    | HitTestFunc  | 注册 HitTest 回调 |   HitTest 坐标   | HitTest 结果 |
    +——————————————+—————————————————+——————————————————+——————————————+
...
local event_handler_pool = 
{
    ...
    [Input.EVENT_MOUSEMOTION_IN_SCREEN] = function() cb_mouse(Input.EVENT_MOUSEMOTION_IN_SCREEN) end,
...
__MODULE__.HitTestFunc = function(callback)
    if not callback then
        Window.StopHitTest()
    else
        assert(type(callback) == "function")
        Window.SetHitTest(callback)
    end
end
